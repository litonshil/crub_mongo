...
type JWTMiddleware struct {
	...
	cacheRepository domain.ICache
}

func NewJWTMiddleWare(redisConfig *config.RedisConfig, cacheRepository domain.ICache) *JWTMiddleware {
...
func (jmr *JWTMiddleware) JWTWithConfig(config JWTConfig) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		...
		return func(c echo.Context) error {
			...
			if err := jmr.cacheRepository.GetStruct(redisConfig.UserPrefix+strconv.Itoa(int(tokenDetails.UserID)), &user); err != nil {
				log.Error(err)
				return ErrJWTMissing
			}
